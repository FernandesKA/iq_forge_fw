...
Конфиг для чтения vendor id AD9361 (`hal::spi_config`) — путь к spidev-ноде
AD9361 и параметры транспорта. Подхватывается `deploy.sh` в архив
автоматически, если файл есть в этой папке; `iq_forge_app` ищет
`./spi.json` в текущей директории (без флагов — путь фиксирован).

`device: /dev/spidev0.0` — **проверено на железе** (20.08.2026, плата
192.168.0.7): после применения оверлея появляется ровно `/dev/spidev0.0`,
...
```sh
./iq_forge_app menu   # пункт 10) apply overlay -> name=rk7020f, dtbo=rk7020f.dtbo
